...
class TrainDataset(Dataset):
    def __init__(self, root, transform=None):
        ...
        IMG_EXTENSIONS = ('.jpg', '.jpeg', '.png',
                          ...
                          '.tiff', '.webp')
        self.images = []
        for img_file in files_under_root:
    ...
    def __getitem__(self, item):
        ...
        cur_img = Image.open(img_path).convert("RGBA")
        background = Image.new("RGBA", cur_img.size,
                               (255, 255, 255))
        alpha_composite = Image.alpha_composite(background, cur_img)

        # Check the transform
        if self.transform is not None:
            cur_img = self.transform(alpha_composite)
